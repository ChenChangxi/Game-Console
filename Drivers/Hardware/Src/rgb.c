...
void rgb_init(void) {
    ...
    rgb_init_handler.Init.HorizontalSync     =                       /* 水平同步宽度（HS）*/
    rgb_init_handler.Init.VerticalSync       =                       /* 垂直同步宽度（VS）*/
    rgb_init_handler.Init.AccumulatedHBP     =                       /* 水平后廊宽度（HS+DE）*/
    rgb_init_handler.Init.AccumulatedVBP     =                       /* 垂直后廊宽度（VS+DE）*/
    rgb_init_handler.Init.AccumulatedActiveW =                       /* 行数据有效字（HS+DE+DE）*/
    rgb_init_handler.Init.AccumulatedActiveH =                       /* 帧数据有效行（VS+DE+DE）*/
    rgb_init_handler.Init.TotalWidth         =                       /* 水平前廊宽度（HS+DE+DE+DE）*/
    rgb_init_handler.Init.TotalHeigh         =                       /* 垂直前廊宽度（VS+DE+DE+DE）*/
    ...
    rgb_layer_handler.FBStartAdress   =                              /* 显存起始地址 */
    rgb_layer_handler.ImageWidth      =                              /* 显存行长 */
    rgb_layer_handler.ImageHeight     =                              /* 显存列长 */
    ...
    HAL_LTDC_ConfigLayer(&rgb_init_handler, &rgb_layer_handler, 0);
}
