...
uint8_t rom_ders[] = "已擦除\r\n";
uint8_t rom_dout[] = "数据溢出\r\n";
uint8_t rom_addr;
uint8_t rom_erse[256] = {0xff};

void at24c02_init(void) {rom_read(0xff, &rom_addr, 1);}
...
void rom_write(uint8_t addr, uint8_t *buff, uint16_t size) {while (size--) rom_write_byte(addr++, *buff++);}

void rom_read(uint8_t addr, uint8_t *buff, uint16_t size) {while (size--) *buff++ = rom_read_byte(addr++);}